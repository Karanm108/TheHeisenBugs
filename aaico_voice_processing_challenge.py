...
def process_data():
    ...
    while i != number_of_frames:
        ...
        ### Array of the commands samples to be detected
        command_samples = [
        [142000, 160000],
        [340000, 360000],
        [620000, 635000]
        ]
        ## Loop going through the samples of each frame
        for j in range(len(list_samples_id)):
            ## checking if the sample is in the range of the command samples
            if any([list_samples_id[j] >= sample[0] and list_samples_id[j] <= sample[1] for sample in command_samples]):
                ## Changing the label of the sample to 0
              labels[j]=0
